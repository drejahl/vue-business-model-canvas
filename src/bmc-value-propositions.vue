<template>
  <v-card color="grey lighten-2">
    <v-toolbar flat color="grey darken-1" dark height="40px">
      ...
      <v-toolbar-title style="margin-left: 0px;" class="body-1">Value Propositions</v-toolbar-title>
    </v-toolbar>
    <v-layout row wrap style="min-height: 50px;">
      <v-flex xs12 v-for="(p,i) in propositions" :key="p.id">
        <v-card color="teal lighten-2" class="white--text">
          <v-toolbar flat color="teal lighten-1" dark height="20px">
            ...
            <v-btn v-if="enableComments" flat dark icon small style="margin-left: 0px;" @click.native.stop="$emit('newPropositionComment', p)">
              <v-icon size="16px">add_comment</v-icon>
            </v-btn>
            <v-btn v-if="enableComments && p.id" flat dark icon small style="margin-left: 0px;" @click.native.stop="$emit('newPropositionRating', p)">
    ...
    <v-dialog v-model="dialog" persistent max-width="500px">
      <v-card>
        <v-card-title>
          <span class="headline">Value Proposition</span>
        </v-card-title>
        <v-card-text>
          <v-container grid-list-md>
            <v-layout wrap>
              <v-flex xs12 sm12 md8>
                <v-text-field label="Name" required v-model="proposition.name"></v-text-field>
              </v-flex>
              <v-flex xs12 sm6>
                <v-select label="Type" multiple autocomplete chips v-model="proposition.type"
                  :items="['Service', 'Product']">
                </v-select>
              </v-flex>
              <v-flex xs12 sm12 md12>
                <v-text-field multi-line label="Description" v-model="proposition.description"></v-text-field>
              </v-flex>
            </v-layout>
          </v-container>
        </v-card-text>
...
export default {
  ...
  methods: {
    ...
    create: function() {
      this.proposition = { type: [] };
      this.dialog=true;
    },
    edit: function(i) {
      this.proposition =  Object.assign( {}, this.propositions[i]);
      this.editIdx = i;
      this.dialog=true;
    },
    save: function(i) {
      ...
      if (!this.proposition.id) {
        this.proposition.id=uuidv1();
        this.propositions.push(this.proposition);
      } else {
        this.propositions[this.editIdx] = Object.assign( {}, this.proposition);
      }
    }
  }
}
